...
$data = query("SELECT * FROM home WHERE id=$ubah")[0];

if(isset($_POST['ubah'])){
     //mengecek data berhasil ditambahkan atau tidak
     if(ubah($_POST) > 0){
        echo "<script>alert('data anda berhasil dirubah');document.location.href='viewpengumuman.php';</script>";
    }else{
        echo "<script>alert('data anda Gagal dirubah');document.location.href='viewpengumuman.php';</script>";
    }
    ...
    <input type="hidden" name="gambarlama" id="id" value="<?= $data["gambar"] ?>">
        <ul>
            <li>
                ...
                <input class="tgl"type="text" id="tgl" name="judul" value="<?= $data['judul']; ?>">
            </li>
            <li>
                <label for="isi">Masukkan pengumuman disini :</label>
                 <input class="isi"type="text" name="pengumuman" id="isi"  value="<?= $data['pengumuman'];?>" >
            </li>
            <li>
